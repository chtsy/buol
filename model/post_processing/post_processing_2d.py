...
def merge_semantic_and_instance(sem_seg, ins_seg, label_divisor, thing_list, stuff_area, void_label):
    ...
    for ins_id in instance_ids:
        ...
        if class_id.item() in class_id_tracker:
            ...
            new_ins_id = 1
        class_id_tracker[class_id.item()] += 1
        ins_class_ids.update({int(ins_id):int(class_id)})
...
def get_panoptic_segmentation(sem, ctr_hmp, offsets, thing_list, label_divisor, stuff_area, void_label,
    ...
    instance, center = get_instance_segmentation(semantic, ctr_hmp, offsets, thing_list,
                                                 ...
                                                 thing_seg=thing_seg)
    panoptic, class_ids = merge_semantic_and_instance(semantic, instance, label_divisor, thing_list, stuff_area, void_label)

    center_class_mapping = []
    center_mapped = []
    for ins_id in range(1, len(center[0])+1):
        if ins_id in class_ids:
            center_mapped.append(center[:,ins_id-1:ins_id])
            center_class_mapping.append(class_ids[ins_id])
    if len(center_mapped) != 0:
        center_mapped = torch.cat(center_mapped, 1)
    else:
        center_mapped = torch.zeros_like(center)
    center_class_mapping = torch.tensor(center_class_mapping,device=panoptic.device)

    return panoptic, center_mapped, center_class_mapping
