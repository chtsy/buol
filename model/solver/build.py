...
def build_optimizer(config, model, decay=1.):
    ...
    for module in model.modules():
        for key, value in module.named_parameters(recurse=False):
            ...
            if value in memo:
                continue
            memo.add(value)
            lr = config.SOLVER.BASE_LR * decay
            ...
            if isinstance(module, norm_module_types):
                ...
                weight_decay = config.SOLVER.WEIGHT_DECAY_BIAS * decay
    ...
    if config.SOLVER.OPTIMIZER == "sgd":
        ...
        optimizer = torch.optim.Adam(params, config.SOLVER.BASE_LR, betas=config.SOLVER.ADAM_BETAS,
                                     weight_decay=config.SOLVER.WEIGHT_DECAY, eps=config.SOLVER.ADAM_EPS)
    else:
