...
class DepthMap(object):
    ...
    def compute_normal(self):
        ...
        yv = yv.reshape(-1).float() * depth_map.float() ##.cuda()
        xv = xv.reshape(-1).float() * depth_map.float() ##.cuda()
        coords3d = torch.stack([xv, yv, depth_map.float(), torch.ones_like(depth_map).float()]) ##.cuda()
        pointcloud = torch.mm(torch.inverse(self.intrinsic_matrix.float()), coords3d.float()).t()[:, :3] ##.cuda()
